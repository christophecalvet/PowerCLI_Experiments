...
$Result = (([appdomain]::currentdomain.GetAssemblies()).Modules | Where-Object {$_.Name -eq 'vmware.vim.dll'}).Assembly.modules.gettypes() | foreach-object{
 $TypeName = $_.Name
 $TypeFullName = $_.FullName
 $TypeBaseType = $_.BaseType
 $TypeIsEnum = $_.IsEnum
	 $_.getproperties() | foreach-object{
										$Output = New-Object -Type PSObject -Prop ([ordered]@{
										'TypeName'=  $TypeName
										'TypeFullName' = $TypeFullName
										'TypeBaseType' =  $TypeBaseType
										'TypeIsEnum' = $TypeIsEnum 
										'Name' = $_.Name
										'DeclaringType' = $_.DeclaringType
										'ReflectedType' = $_.ReflectedType
										'PropertyType' = $_.PropertyType
...
$Result | Export-Csv -path C:\Temp\AllPropertiesPerType.csv -delimiter "`t" -notypeinformation
			...
			#Extract all methods with the exception of the method to capture or set properties
			$xmlWriter.WriteStartElement("Methods")
			$ObjectToAnalyse.getmembers() | where {$_.MemberType -eq "Method"} | where {$_.Name -notmatch "get_" -AND $_.Name -notmatch "set_" -AND $_.Name -ne "ToString" -AND $_.Name -ne "Equals" -AND $_.Name -ne "GetHashCode" -AND $_.Name -ne "GetType"} | foreach-object{
				$xmlWriter.WriteStartElement($_.Name)
				$xmlWriter.WriteEndElement()
			}
			$xmlWriter.WriteEndElement() #Matching the start element "Methods"
												...
												elseif ($Global:AllVMwareVimObject| where {$_.baseType.fullname -eq $PropertytypeFiltered}){
													$xmlWriter.WriteStartElement($PropertytypeFiltered)
													$xmlWriter.WriteStartElement("ExtendedBy")
																			$Global:AllVMwareVimObject | where {$_.baseType.fullname -eq $PropertytypeFiltered} | foreach{
																			ExportAPIRecursiveTo-XML -ObjectToAnalyse $_  -fullpath $fullpath2
																			}
													$xmlWriter.WriteEndElement()
													$xmlWriter.WriteEndElement()
												}
												...
												<#	ForEach($MyType in ((([appdomain]::currentdomain.GetAssemblies()).Modules | Where-Object {$_.Name -eq 'vmware.vim.dll'}).Assembly.modules.gettypes() | Where-Object {$_.FullName -eq $SpecificTypeFullName})){
	 $ClassDefintion = $MyType.GetMembers()
	 }	
#>				 

	<#
	$NewCOnnector = $page.DropIntoList($ShapeSeparator,$MyNewClass,9999999)
	 
	 ForEach($MyMember in $ClassDefintion){
		If ($MyMember.MemberType -eq "Method"){